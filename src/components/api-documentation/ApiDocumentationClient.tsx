...
export function ApiDocumentationClient({
  ...
  return (
    <div className="api-status-page">
      <header
        {...journalStaggerRowProps(0, { className: "api-status-page-header" })}
      >
        <FileText
          ...
          <TabsContent value="overview">
            <Card>
              ...
              <CardContent className="space-y-4 font-lora text-sm leading-relaxed text-[rgba(255,205,130,0.7)]">
                <p>
                  <strong className="text-[rgba(255,210,140,0.9)]">
                    Rendering:
                  </strong>{" "}
                  SSR in <code>page.tsx</code> with <code>force-dynamic</code>;
                ...
                <p>
                  ...
                  TanStack Query with <code>initialData</code> from server.
                  Journal mutations invalidate via{" "}
                ...
                <p>
                  ...
                  SSE at <code>GET /api/journal/events</code> polls Redis
                  buffer; cross-tab sync triggers cache invalidation without
                  page refresh.
                </p>
                <p>
                  <strong className="text-[rgba(255,210,140,0.9)]">
                  ...
                  Session-scoped routes filter all Prisma queries by{" "}
                  <code>session.user.id</code>. Public routes:{" "}
                  <code>/api/health</code>, <code>/api/auth/register</code>,
                  NextAuth handlers.
                </p>
                <p>
                  <strong className="text-[rgba(255,210,140,0.9)]">
                  ...
                  Journal APIs return{" "}
                  <code>{`{ success, data, message? }`}</code>. AI routes use{" "}
                ...
                <div className="mt-4">
                  ...
                  <ul className="space-y-2">
                    {catalog.tags.map((t) => (
                      <li key={t.name}>
                        <span className="font-medium text-[rgba(255,210,140,0.85)]">
                          {t.name}
                        </span>
                        {" — "}
                        {t.description}
